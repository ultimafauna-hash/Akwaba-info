  ...
  return (
    <div className="fixed inset-0 z-[5000] flex items-center justify-center p-4">
      ...
      <motion.div
        ...
        className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl w-full max-w-md overflow-hidden relative flex flex-col max-h-[90vh]"
      >
        ...
        <div className="p-6 md:p-8 overflow-y-auto custom-scrollbar">
          {isVerificationPending ? (
            <div className="text-center space-y-6 py-4">
              <div className="w-20 h-20 bg-emerald-100 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 rounded-full flex items-center justify-center mx-auto animate-bounce-slow">
                <Mail size={40} />
              </div>
              <div className="space-y-2">
                <h3 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tight">Vérifiez votre boîte mail</h3>
              ...
              <div className="pt-4">
                ...
                <button
                  onClick={onClose}
                  className="w-full py-4 mt-2 text-slate-400 dark:text-slate-500 font-bold text-xs uppercase tracking-widest hover:text-slate-600 dark:hover:text-slate-300 transition-colors"
                >
                  Fermer
                </button>
              </div>
            ...
            <>
              ...
              {error && (
                <div className="mb-6 p-4 bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800 rounded-2xl flex items-start gap-3 animate-shake">
                  <AlertTriangle className="text-red-500 dark:text-red-400 shrink-0 mt-0.5" size={18} />
                  <p className="text-red-700 dark:text-red-300 text-xs font-bold leading-tight">{error}</p>
                </div>
              )}

              {successMessage && (
                <div className="mb-6 p-4 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800 rounded-2xl flex items-start gap-3">
                  <CheckCircle className="text-emerald-500 dark:text-emerald-400 shrink-0 mt-0.5" size={18} />
                  <p className="text-emerald-700 dark:text-emerald-300 text-xs font-bold leading-tight">{successMessage}</p>
                </div>
              )}
            ...
            {mode === 'register' && (
              <div className="space-y-1.5">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 ml-4">Nom Complet</label>
                <div className="relative group">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-4 py-4 bg-slate-50 dark:bg-slate-800 border-2 border-transparent focus:border-primary/20 dark:focus:border-primary/40 focus:bg-white dark:focus:bg-slate-800 rounded-2xl text-sm font-bold transition-all outline-none dark:text-white"
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 ml-4">Email</label>
              <div className="relative group">
            ...
            {mode !== 'forgot-password' && (
              <div className="space-y-1.5">
                <div className="flex items-center justify-between px-4">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">Mot de passe</label>
                  {mode === 'login' && (
                    <button 
                      type="button"
                ...
                <div className="relative group">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-12 py-4 bg-slate-50 dark:bg-slate-800 border-2 border-transparent focus:border-primary/20 dark:focus:border-primary/40 focus:bg-white dark:focus:bg-slate-800 rounded-2xl text-sm font-bold transition-all outline-none dark:text-white"
                    placeholder="••••••••"
                  />
                  <button
                    type="button"
